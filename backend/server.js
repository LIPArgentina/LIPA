...
// Corrección puntual y reversible en producción: conservar el Oldies original
// (ID 319, con plantel) y retirar el duplicado vacío (ID 1126).
(async () => {
  const client = await pool.connect();
  const fixKey = "2026-07-31-oldies-production-canonical-319";
  ...
  try {
    ...
    await client.query("ROLLBACK");
    console.error("No se pudo unificar Oldies en producción:", err);
  } finally {
    client.release();
  }
})();
