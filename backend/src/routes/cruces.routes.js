...
router.get('/status', async (req, res) => {
  try {
    ...
    if (category) {
      const syntheticFechaKey = new Date().toISOString().slice(0, 10);
      const state = getCrucesEntry(team, syntheticFechaKey);

      return res.json({
        ok: true,
        team,
        category,
        fechaKey: syntheticFechaKey,
    ...
    if (!fechaKey) {
      return res.status(400).json({ ok: false, error: 'Falta parámetro fechaKey.' });
    }
    ...
    console.error('GET /cruces/status', error);
